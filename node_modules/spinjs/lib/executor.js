            ...
            if (!nodeDebugger) {
                ...
                child_process_1.exec('node -v', function (error, stdout, stderr) {
                    ...
                    detectPort(9229).then(function (debugPort) {
                        spawnServer(cwd, [nodeDebugOpt + '=' + debugPort, serverPath], { serverPath: serverPath, nodeDebugger: nodeDebugger }, logger);
                    });
                });
            }
        }
...
var startWebpackDevServer = function (hasBackend, spin, builder, reporter, logger) {
    ...
    if (platform === 'web') {
        ...
        var symbolicateMiddleware = builder.require('haul/src/server/middleware/symbolicateMiddleware');
...
var mirrorExpoLogs = function (builder, projectRoot) {
    var ProjectUtils = builder.require('xdl').ProjectUtils;
    deviceLoggers[projectRoot] = minilog('expo-for-' + builder.name);
    if (!ProjectUtils.logWithLevel._patched) {
        var origExpoLogger_1 = ProjectUtils.logWithLevel;
        ProjectUtils.logWithLevel = function (projRoot, level, object, msg, id) {
            var json;
            if (msg[0] === '{') {
                json = JSON.parse(msg);
            }
            if (level === 'error') {
                var info = object.includesStack ? json.message + '\n' + json.stack : json.message;
            ...
            return origExpoLogger_1.call(ProjectUtils, projRoot, level, object, msg, id);
        };
        ...
        if (cmd === 'exp') {
            ...
            for (var _i = 0, _a = Object.keys(builders); _i < _a.length; _i++) {
                ...
                if (builder.roles.indexOf('test') >= 0) {
                    ...
                    var testCmd = path.join(process.cwd(), 'node_modules/.bin/mocha-webpack');
